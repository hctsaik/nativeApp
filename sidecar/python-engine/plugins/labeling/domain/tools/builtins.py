...
if TYPE_CHECKING:
    from annotation.tools.registry import ToolRegistry
...
class _XAnyLabelingAdapter:
    ...
    def detect(self, executable_override: str | None = None) -> RuntimeStatus:
        from annotation.adapters.xanylabeling_runtime import detect_xanylabeling
        if executable_override and Path(executable_override).exists():
            from annotation.adapters.xanylabeling_runtime import _read_version
    ...
    def launch_project(self, project_dir: Path, options: dict) -> dict:
        ...
        from annotation.adapters.xanylabeling_runtime import launch_xanylabeling_project
        if exe_override:
