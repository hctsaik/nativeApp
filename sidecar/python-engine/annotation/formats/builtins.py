...
if TYPE_CHECKING:
    from annotation.core.models import AdapterResult, AnnotationSet, ConversionReport, ImageAsset, LabelSchema
    from annotation.formats.registry import FormatRegistry
...
class _LabelMeAdapter:
    ...
    def export(self, annotation_set, schema, assets, output_dir, *, dry_run=False):
        from annotation.adapters.labelme import export_labelme
        if dry_run:
    ...
    def import_file(self, path, dataset_id, schema, asset, all_assets):
        from annotation.adapters.labelme import import_labelme_file
        return import_labelme_file(path, dataset_id, schema, asset)

    def import_dir(self, path, dataset_id, schema, assets):
        from annotation.adapters.labelme import import_labelme_project_dir
        return import_labelme_project_dir(path, dataset_id, schema, assets)


class _IsatAdapter:
    def export(self, annotation_set, schema, assets, output_dir, *, dry_run=False):
        from annotation.adapters.isat import export_isat
        if dry_run:
            with tempfile.TemporaryDirectory() as tmp:
    ...
    def import_file(self, path, dataset_id, schema, asset, all_assets):
        from annotation.adapters.isat import import_isat_file
        return import_isat_file(path, dataset_id, schema, asset)

    def import_dir(self, path, dataset_id, schema, assets):
        from annotation.adapters.isat import import_isat_project_dir
...
class _CocoAdapter:
    ...
    def import_file(self, path, dataset_id, schema, asset, all_assets):
        from annotation.adapters.coco import import_coco_file
...
class _YoloDetectionAdapter:
    ...
    def import_dir(self, path, dataset_id, schema, assets):
        from annotation.adapters.yolo_detection import import_yolo_detection_dir
        return import_yolo_detection_dir(path, dataset_id, schema, assets)


class _YoloSegmentationAdapter:
    def export(self, annotation_set, schema, assets, output_dir, *, dry_run=False):
        from annotation.adapters.yolo_segmentation import export_yolo_segmentation
        if dry_run:
            with tempfile.TemporaryDirectory() as tmp:
    ...
    def import_dir(self, path, dataset_id, schema, assets):
        from annotation.adapters.yolo_segmentation import import_yolo_segmentation_dir
