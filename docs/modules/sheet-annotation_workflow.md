...
| Runner | `sheet_runner` |
| 主要模組 | module_010 → 012 → 013 → 014 → 015 → 016 |
...
module_014  📤 Export
  ...
  └─ 寫 annotation_exports 記錄（供 Dashboard 讀取）
        │
        ▼
module_015  📊 Dashboard
  ├─ 標注進度（BBox / 分類完成度 + 進度條）
...
| 檔案 | 責任 |
|------|------|
| `{CIM_LOG_DIR}/config/shared.json` | module_010 寫入最新 manifest；012/013/014/015/016 讀取 |
...
| `{CIM_LOG_DIR}/config/module_012_classifications_{manifest_id[:12]}.json` | 分類結果；012 寫、013/014/015/016 讀 |
