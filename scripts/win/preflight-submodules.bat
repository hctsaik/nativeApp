...
setlocal
rem ============================================================================
rem  CIM submodule preflight  --  detect whether git submodules are initialized
rem ----------------------------------------------------------------------------
...
rem  exit /b 0 = ok ; exit /b 1 = submodule content missing (caller should abort)
rem ============================================================================
set "REPO_ROOT=%~dp0..\.."
...
echo.
echo ============================================================
echo  [CIM-PREFLIGHT] git submodules are NOT initialized.
echo ============================================================
echo.
echo  Symptom : the workflow list is missing items (Labeling / AI Report)
echo            and/or the app fails to start.
echo.
echo  Cause   : obtained via GitHub "Download ZIP", or cloned without
echo            --recurse-submodules. ZIP / shallow clone do NOT contain
echo            submodule content.
echo.
echo  Missing :
if not exist "%LABELING_SENTINEL%" echo            - Labeling   submodule: plugins\labeling   repo: ANnoTation
if not exist "%AI4BI_SENTINEL%"   echo            - AI Report  submodule: vendor\AI4BI       repo: AI4BI
echo.
echo  FIX (run at the repo root):
echo            git submodule update --init --recursive
echo.
echo  If you used ZIP download, clone with submodules instead:
